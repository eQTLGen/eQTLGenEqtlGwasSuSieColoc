...
workflow {
       ...
       */
       ...
       MAKEANNOTATIONTABLE.out
        ...
        coloc_input_ch = MAKEANNOTATIONTABLE.out.combine(finemapped_gwas_ch).combine(ld_ch).combine(coloc_pp4_ch).combine(full_output_ch).combine(mrlink2_ch).combine(PARSEGENENAMES.out).combine(allele_ch).combine(gene_file_tuples_ch2)
